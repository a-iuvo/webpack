...
class RequireResolveDependencyParserPlugin {
	...
	apply(parser) {
		...
		parser.plugin("call require.resolve", (expr) => parser.applyPluginsBailResult("call require.resolve(Weak)", expr, false));
		parser.plugin("call require.resolveWeak", (expr) => parser.applyPluginsBailResult("call require.resolve(Weak)", expr, true));
